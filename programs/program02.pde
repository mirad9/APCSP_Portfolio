...
void draw(){
  background(#41DE00);
  if(animalskilled>5&&speed<25){
  speedboost(speedX,speedY);
  }
  if(animalskilled>15&&range<450){
  ...
  if(spawns1){
    elephantdead=false;
    elephantX=random(hunterX+200+random(700));
    elephantY=random(hunterY+200+random(400));
    spawns1=false;
  }if(shoots){
     shoots=false;
  }if(elephantdead){
...
void fastergun(float x,float y){
  if(animalskilled>12&&!guncooldown){
    guncooldown=true;
  }if(!(hunterX>gunsX+35&&hunterX<gunsX&&hunterY>gunsY+5&&hunterY<gunsY)&&guncooldown){
 ...
 fill(0);
 rect(x+15,y,20,2);
 if(hunterX<gunsX+35&&hunterX>gunsX&&hunterY<gunsY+5&&hunterY>gunsY){
  shotcooldownvalue=shotcooldownvalue-5;
  guncooldown=true;
 }if(shotcooldownvalue<10){
  shotcooldownvalue=10; 
 }
}
...
range=range+50;
rangecooldown=true;
  }if(animalskilled>15&&rangecooldown&&!(hunterX<rangeX+60&&hunterX>rangeX&&hunterY<rangeY+40&&hunterY>rangeY)){
   rangecooldown=false;
   rangeX=random(1000);
   rangeY=random(450);
  }
